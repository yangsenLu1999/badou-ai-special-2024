...
def create_Pnet(weight_path):
    ...
    x = Conv2D(10,(3,3),strides=1,padding='valid',name='conv1')(input) #不填充
    x = PReLU(shared_axes=[1,2],name='PRELU1')(x)
    x = MaxPool2D(pool_size=2)(x)
    x = Conv2D(16, (3, 3), strides=1, padding='valid', name='conv2')(x)  # 不填充
    x = PReLU(shared_axes=[1, 2], name='PRELU2')(x)
    x = Conv2D(32, (3, 3), strides=1, padding='valid', name='conv3')(x)  # 不填充
    x = PReLU(shared_axes=[1, 2], name='PRELU3')(x)
    classifier = Conv2D(2,(1,1),activation='softmax',name='conv4-1')(x)
    box_regress = Conv2D(4,(1,1),name='conv4-2')(x) # 无激活函数，线性。

    # 使用inputs与outputs建立函数链式模型；
    model = Model(inputs=[input],outputs=[classifier,box_regress])
    model.load_weights(weight_path,by_name=True) #by_name代表加载模型中与预训练模型一致部分的参数
    return model
def create_Rnet(weight_path):
    ...
    box_regress=Dense(4,name='conv5-2')(x)
    # 使用inputs与outputs建立函数链式模型；
    model = Model(inputs=[input], outputs=[classifier, box_regress])
    model.load_weights(weight_path, by_name=True)  # by_name代表加载模型中与预训练模型一致部分的参数
    return model
def create_Onet(weight_path):
    ...
    x = PReLU(shared_axes=[1, 2], name='PRELU3')(x)
    x = MaxPool2D(pool_size=2, padding='valid')(x)
    # 4,4,64 -> 3,3,128
    ...
    box_regress = Dense(4, name='conv6-2')(x)
    landmark_regress = Dense(10, name='conv6-3')(x)
    # 使用inputs与outputs建立函数链式模型；
    model = Model(inputs=[input], outputs=[classifier, box_regress,landmark_regress])
    model.load_weights(weight_path, by_name=True)  # by_name代表加载模型中与预训练模型一致部分的参数
    return model
class mtcnn():
    def __init__(self):
