...
def InceptionV3(input_shape=[299, 299, 3], classes=1000):
    # 定义inceptionV3网络结构 类别1000 输入形状是[299, 299, 3]

    img_input = Input(shape=input_shape)

    # 滤波器个数32(即通道数) 卷积核大小3*3 步长2*2 不使用非零补齐 输入尺寸[299, 299, 3] 输出尺寸[149, 149, 32]
    x = conv2d_bn(img_input, 32, 3, 3, strides=(2, 2), padding='valid')
    # 滤波器个数32(即通道数) 卷积核大小3*3 步长1*1 不使用非零补齐 输入尺寸[149, 149, 32] 输出尺寸[147, 147, 32]
    x = conv2d_bn(x, 32, 3, 3, padding='valid')
    # 滤波器个数64(即通道数) 卷积核大小3*3 步长1*1 使用非零补齐 输入尺寸[147, 147, 32] 输出尺寸[147, 147, 64]
    x = conv2d_bn(x, 64, 3, 3)
    # 最大池化 核大小3*3 步长2*2 输入尺寸[147, 147, 64] 输出尺寸[73, 73, 64]
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    # 滤波器个数80(即通道数) 卷积核大小1*1 步长1*1 不使用非零补齐 输入尺寸[73, 73, 64] 输出尺寸[73, 73, 80]
    x = conv2d_bn(x, 80, 1, 1, padding='valid')
    # 滤波器个数192(即通道数) 卷积核大小3*3 步长1*1 不使用非零补齐 输入尺寸[73, 73, 80] 输出尺寸[71, 71, 192]
    x = conv2d_bn(x, 192, 3, 3, padding='valid')
    # 最大池化 核大小3*3 步长2*2  输入尺寸[71, 71, 192] 输出尺寸[35, 35, 192]
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    ...
    # 输入尺寸[35, 35, 192] 输出尺寸[35, 35, 64]
    branch1x1 = conv2d_bn(x, 64, 1, 1)
    # 输入尺寸[35, 35, 64] 输出尺寸[35, 35, 48]
    branch5x5 = conv2d_bn(x, 48, 1, 1)
    # 输入尺寸[35, 35, 48] 输出尺寸[35, 35, 64]
    branch5x5 = conv2d_bn(branch5x5, 64, 5, 5)
    # 输入尺寸[35, 35, 64] 输出尺寸[35, 35, 64]
    branch3x3dbl = conv2d_bn(x, 64, 1, 1)
    # 输入尺寸[35, 35, 64] 输出尺寸[35, 35, 96]
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3)
    # 输入尺寸[35, 35, 96] 输出尺寸[35, 35, 96]
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, 3, 3)
    # 平均池化 核大小3*3 步长1*1 全零填充 输入尺寸[35, 35, 96] 输出尺寸[35, 35, 96]
    branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
    # 输入尺寸[35, 35, 96] 输出尺寸[35, 35, 32]
    branch_pool = conv2d_bn(branch_pool, 32, 1, 1)
    
    # 加和 64+64+96+32 = 256  n h w c - 0 1 2 3
    ...
    for i in range(2):
        ...
        x = layers.concatenate(
            ...
            axis=3,
            name='mixed' + str(9 + i))
    # # block3结束 平均池化后全连接 用softmax输出概率值和预测结果
