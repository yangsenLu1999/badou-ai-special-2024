...
class ProposalLayer(Layer):
    ...
    # 生成建议框，处理输入先验框scores，调整参数和anchors
...
class PyramidROIAlign(Layer):
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            # 找到所有属于当前特征层级别的建议框的索引
            ix = tf.where(tf.equal(roi_level, level))
            level_boxes = tf.gather_nd(boxes, ix) # 根据索引从张量boxes中提取属于当前特征层级别的建议框
            ...
            # 停止梯度下降
            # 这是为了调整和剪裁特定的roi的，不是通过训练得到，而是提取，不需要进入反向传播来调整
            level_boxes = tf.stop_gradient(level_boxes)
            box_indices = tf.stop_gradient(box_indices)

            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            # 在特征图上进行roi对齐之后，将结果添加到pooled列表中
            pooled.append(tf.image.crop_and_resize(
                feature_maps[i], # 当前特征图
                level_boxes, # 要裁剪的框列表
                box_indices, # 每个框所属的批次索引
                self.pool_shape, # 裁剪和调整大小后的输出形状
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    refined_rois = apply_box_deltas_graph(
        rois, deltas_specific * config.BBOX_STD_DEV)  # 边界框回归调整
    # 防止超出0-1
    refined_rois = clip_boxes_graph(refined_rois, window) # 将边界框限制在图像窗口内
...
# 从候选区域和真实标签中生成用于训练的目标
# 选择正负样本，计算边界框回归目标，提取目标掩码
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    """
        生成检测目标，用于训练 Mask R-CNN 模型。
        ...
        target_class_ids: [TRAIN_ROIS_PER_IMAGE] int class IDs. Zero padded.
        target_deltas: [TRAIN_ROIS_PER_IMAGE, (dy, dx, log(dh), log(dw))]
        target_mask: [TRAIN_ROIS_PER_IMAGE, height, width] of boolean type
    """
    asserts = [
