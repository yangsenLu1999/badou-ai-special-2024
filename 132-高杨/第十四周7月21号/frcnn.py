...
import pickle
import os
import nets.frcnn1 as frcnn
from nets.frcnn_training import get_new_img_size
from keras import backend as K
from keras.layers import Input
from keras.applications.imagenet_utils import preprocess_input
from PIL import Image,ImageFont, ImageDraw
from Badou.第十四周7月21号.utils.utils import BBoxUtility
from Badou.第十四周7月21号.utils.anchors import get_anchors
...
class FRCNN(object):
    ...
    def detect_image(self, image):
        ...
        labels = np.array(labels)  # list 61
        probs = np.array(probs)   # list  【61】
        boxes = np.array(bboxes,dtype=np.float32)
        # 映射回原图得框 由于  self.rpn_stride经过RPN网络后输入图像的缩小倍数
        ...
        for i, c in enumerate(top_label_indices):
            ...
            #这行代码使用 ImageDraw 对象的 textsize 方法计算 label 字符串在指定字体中的大小。
            #label 是之前创建的字符串。
            #font 是之前加载的字体对象。
