...
def identity_baa(input_tensor,kernel_size,filters,stage,block,use_bias = True,train_bn=True):
    filters1,filters2,filters3 = filters

    x = Conv2D(filters1,(1,1),use_bias=use_bias)(input_tensor)
    x = BatchNormalization()(x,train_bn=train_bn)
    x = Activation('relu')(x)

    x = Conv2D(filters2,(kernel_size,kernel_size),padding='same',use_bias=use_bias)(x)
    x = BatchNormalization()(x,train_bn=train_bn)
    x = Activation('relu')(x)

    x = Conv2D(filters3, (1, 1), padding='same', use_bias=use_bias)(x)
    x = BatchNormalization()(x, train_bn=train_bn)
    x = Activation('relu')(x)
