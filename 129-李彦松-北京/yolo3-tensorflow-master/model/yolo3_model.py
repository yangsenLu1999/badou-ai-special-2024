...
class yolo:
    ...
    def _get_anchors(self):
        ...
        with open(classes_path) as f: # 读取anchors文件
            anchors = f.readline()
        anchors = np.array(anchors.split(','), dtype = np.float32)
        return anchors.reshape(-1, 2) #重塑为一个二维数组，其中每一行包含两个元素。
    ...
    def _Residual_block(self, inputs, filters_num, blocks_num, conv_index, training = True, norm_decay = 0.99, norm_epsilon = 1e-3):
        ...
        inputs = tf.pad(inputs, paddings=[[0, 0], [1, 0], [1, 0], [0, 0]], mode='CONSTANT') # 为了保证长宽一致，把第二维和第三维的前面各填充1
        ...
        conv_index += 1
        for _ in range(blocks_num): #先进行1X1的卷积，再进行3X3的卷积，最后把这个结果加上layer作为最后的结果
            shortcut = layer
