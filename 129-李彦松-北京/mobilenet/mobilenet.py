...
def MobileNet(input_shape=[224,224,3],
              depth_multiplier=1, # 设置为1可以使第一次卷积只有一个通道，让第二次卷积通道数，减小计算量
              dropout=1e-3,
              classes=1000):
    ...
    # 7,7,1024 -> 1,1,1024
    x = GlobalAveragePooling2D()(x) # 将张量展平
...
def _conv_block(inputs, filters, kernel=(3, 3), strides=(1, 1)):
    x = Conv2D(filters, kernel,
               ...
               use_bias=False,
               strides=strides,
               name='conv1')(inputs) #()里面是默认参数，表示
    x = BatchNormalization(name='conv1_bn')(x)
    return Activation(relu6, name='conv1_relu')(x)

# 深度可分离卷积，先用3×3卷积hw，再用1×1卷积c
...
def relu6(x): # relu函数，小于0的部分截断为0，大于6的部分截断为6
...
if __name__ == '__main__':
    ...
    str(model.summary())   # 打印模型结构,str()函数将对象转化为适于人阅读的形式
    ...
    print(np.argmax(preds)) # 最大的概率值的索引
