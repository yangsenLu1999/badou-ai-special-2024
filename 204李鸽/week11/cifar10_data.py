# 数据给到神经网络之前，要哦先做预处理，包括数据类型的转换，数据拆分，更重要的是做图像增强，除了之前学的降噪之外，还有下面会看到
# 图像增强又两个维度：数量和质量
...
def read_cifar10(file_queue):     # 这块相当于手写了，工程中都可调接口，这好复杂
    result = CIFAR10Record()

    label_bytes = 1  # 如果是Cifar-100数据集，则此处为2，1表示用10类的cifar10,占位的话0-9就够了，1位数字
    result.height = 32  # 这块以后不哟个写死，以后用数据.shape就能提取
    result.width = 32
...
def inputs(data_dir,batch_size,distorted):               #这个函数就对数据进行预处理---对图像数据是否进行增强进行判断，并作出相应的操作
    filenames=[os.path.join(data_dir,"data_batch_%d.bin"%i)for i in range(1,6)]   #拼接文件地址，命名为filenames
    ...
    num_examples_per_epoch=num_examples_pre_epoch_for_train    #上面外面已经定义好了


    if distorted != None:                         #如果预处理函数中的distorted参数不为空值，就代表要进行图片增强处理
        # 调用inputs函数的时候取设定，可以是0，1，true,false
        # 以下每一步操作，都会使图像数量翻倍，猫的特征不会因为这些操作而改变，比如白天的猫和晚上的猫
        # 图像数量增加，但标签不变
        ...
        float_image.set_shape([24,24,3])                  # 这两部是设置了图像输入，不是图像增强    #设置图片数据及标签的形状
        read_input.label.set_shape([1])

        min_queue_examples=int(num_examples_pre_epoch_for_eval * 0.4)   # 这步是设置batch数，命名不好
        ...
        images_train,labels_train=tf.train.shuffle_batch([float_image,read_input.label],batch_size=batch_size,
                             ...
                             #使用tf.train.shuffle_batch()洗牌函数随机产生一个batch的image和label，先做哪个lable
        ...
        float_image.set_shape([24,24,3])   # 设置好了网络结构的输入尺寸
        read_input.label.set_shape([1])
