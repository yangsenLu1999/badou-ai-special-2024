...
def variable_with_weight_loss(shape,stddev,w1):   # 这个函数是给权重赋值的
    var=tf.Variable(tf.truncated_normal(shape,stddev=stddev))
    if w1 is not None:
        weights_loss=tf.multiply(tf.nn.l2_loss(var),w1,name="weights_loss")
        tf.add_to_collection("losses",weights_loss)    # 计算loss
    return var
...
#创建第一个卷积层 shape=(kh,kw,ci,co),一个卷积block
kernel1=variable_with_weight_loss(shape=[5,5,3,64],stddev=5e-2,w1=0.0)    # 生成权重，和kernel大小一样，大小5*5，inp3,oup64
conv1=tf.nn.conv2d(x,kernel1,[1,1,1,1],padding="SAME")     # stide:1111,上下左右都一样，做第一次卷积
bias1=tf.Variable(tf.constant(0.0,shape=[64]))     # 生成偏执
relu1=tf.nn.relu(tf.nn.bias_add(conv1,bias1))      # 加法
pool1=tf.nn.max_pool(relu1,ksize=[1,3,3,1],strides=[1,2,2,1],padding="SAME")     # size:3*3,stride2*2<那两个1可以省略

#创建第二个卷积层
kernel2=variable_with_weight_loss(shape=[5,5,64,64],stddev=5e-2,w1=0.0)   # 第二个卷积层inp层=第一个卷积的oup
conv2=tf.nn.conv2d(pool1,kernel2,[1,1,1,1],padding="SAME")
bias2=tf.Variable(tf.constant(0.1,shape=[64]))
relu2=tf.nn.relu(tf.nn.bias_add(conv2,bias2))
pool2=tf.nn.max_pool(relu2,ksize=[1,3,3,1],strides=[1,2,2,1],padding="SAME")

# 定义了两个卷积block之后，开始全连接，先拍扁成1维
...
dim=reshape.get_shape()[1].value             #get_shape()[1].value表示获取reshape之后的第二个维度的值，全连接输入节点的个数
...
result=tf.add(tf.matmul(local4,weight3),fc_bias3)   # 全连接层跟tf一样
...
loss=tf.reduce_mean(cross_entropy)+weights_with_l2_loss    # 计算loss

train_op=tf.train.AdamOptimizer(1e-3).minimize(loss)       # 做一个优化项，用的是adam，然后输入参数learning rate

#函数tf.nn.in_top_k()用来计算输出结果中top k的准确率，函数默认的k值是1，即top 1的准确率，也就是输出分类准确率最高时的数值
top_k_op=tf.nn.in_top_k(result,y_,1)   # 把result排序，1：只输出top1的准确率，5：只输出top5的准确率

# 后面就是session run了，
init_op=tf.global_variables_initializer()
with tf.Session() as sess:
    sess.run(init_op)    # 先run变量的init op
...
#每隔100step会计算并展示当前的loss、每秒钟能训练的样本数量、以及训练一个batch数据所花费的时间
    for step in range (max_steps):   # max_steps实际上就是epotch
        start_time=time.time()
        image_batch,label_batch=sess.run([images_train,labels_train])   # 训练
        _,loss_value=sess.run([train_op,loss],feed_dict={x:image_batch,y_:label_batch})  # 输入最后的计算图，计算Loss
        duration=time.time() - start_time    # 每次循环的时间
