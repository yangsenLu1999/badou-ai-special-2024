...
def conv_block(input, filters, kernel_size, stage, block, strides=(2, 2)):
    filter1, filter2, filter3 = filters
    conv_name_base = 'res' + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(stage) + block + '_branch'

    x = Conv2D(filter1, (1, 1), strides=strides, name=conv_name_base+'2a')(input)
    ...
    shortcut = Conv2D(filter3, (1, 1), strides=strides, name=conv_name_base+'1')(input)
...
def identity_block(input, filters, kernel_size, stage, block):
    filter1, filter2, filter3 = filters
    conv_name_base = 'res' + str(stage) + block + '_branch'
    bn_name_base = 'bn' + str(stage) + block + '_branch'

    x = Conv2D(filter1, (1, 1), name=conv_name_base+'2a')(input)
    x = BatchNormalization(name=bn_name_base+'2a')(x)
    x = Activation('relu')(x)
