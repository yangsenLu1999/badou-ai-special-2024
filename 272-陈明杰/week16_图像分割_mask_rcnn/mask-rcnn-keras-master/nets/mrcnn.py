...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # x=PyramidROIAlign([pool_size,pool_size],
    #                   name='roi_algin_classifier')([rois,image_meta]+feature_maps)
    ...
    # mrcnn_class_logits=TimeDistributed(Dense(num_classes),
    #                                    name='mrcnn_class_logits')(shared)
    # mrcnn_probs=TimeDistributed(Activation(activation='softmax'),
    #                             name='mrcnn_class')(mrcnn_class_logits)
    ...
    # x=TimeDistributed(Dense(num_classes*4,activation='linear'),
    #                   name='mrcnn_bbox_fc')(shared)
    # mrcnn_bbox=Reshape(target_shape=(-1,num_classes,4),name='mrcnn_bbox')(x)
...
def get_predict_model(config):
    ...
    # rpn_rois是rpn中进行解码并初筛后得到的框，mrcnn_class是对于每个框的分类结果
    # mrcnn_bbox是再一次经过微调后得到的更加准确的预测框
    # 这里为什么还要后处理？
