...
def create_Pnet(weight_path):
    ...
    x = Conv2D(filters=10, kernel_size=(3, 3), strides=1, padding='valid')(inputs)
    ...
    classes = Conv2D(filters=2, kernel_size=1, strides=1, padding='valid', activation='softmax')(x)
    prob = Conv2D(filters=4, kernel_size=(1, 1), strides=1, padding='valid')(x)

    model = Model([inputs], [classes, prob])
...
#     # 24,24,3 -> 11,11,28
#     # new_size=(old_size+2p-kernel_size)/s+1
...
def create_Rnet(weight_path):
    ...
    x = Conv2D(filters=28, kernel_size=(3, 3), strides=1, padding='valid')(inputs)
    x = PReLU(shared_axes=[1, 2])(x)
    x = MaxPool2D(pool_size=(3, 3), strides=2, padding='same')(x)

    x = Conv2D(filters=48, kernel_size=(3, 3), strides=1, padding='valid')(x)
    x = PReLU(shared_axes=[1, 2])(x)
    x = MaxPool2D(pool_size=(3, 3), strides=2, padding='valid')(x)

    x = Conv2D(filters=64, kernel_size=(2, 2), strides=1, padding='valid')(x)
    x = PReLU(shared_axes=[1, 2])(x)
    ...
    classes = Dense(2, activation='softmax')(x)
    bbox_prob = Dense(4)(x)

    model = Model([inputs], [classes, bbox_prob])
    model.load_weights(weight_path)
    return model
...
def create_Onet(weight_path):
    ...
    x = Conv2D(filters=128, kernel_size=(2, 2), strides=1, padding='valid')(x)
    x = PReLU(shared_axes=[1, 2])(x)
    ...
    classes = Dense(2, activation='softmax')(x)
    bbox_prob = Dense(4)(x)
    Facial_landmark = Dense(10)(x)

    model = Model([inputs], [classes, bbox_prob, Facial_landmark])
    model.load_weights(weight_path)
    return model
...
class mtcnn():
    ...
    def detectFace(self, img, threshold):
        ...
        for i in range(image_num):
            # 有人脸的概率
            # 每一个out[i]是一个列表，表示Pnet网络的两个输出头，out[i][0]表示第一个头
            # 是用于判断边框是否有人脸的，out[i][0][0]中的后一个0其实是表示batch维度，
            # 即表示某一张图片，[:,:]表示这张图片的所有的预测框，[1]表示框中是否有人脸以及
            # 预测框的置信度是多少
        ...
        for rectangle in rectangles:
            if rectangle is not None:
                ...
                for i in range(4, len(rectangle), 2):  # 从索引4开始，每次增加2
                    if i + 1 < len(rectangle):  # 确保i+1不会超出rectangle的范围
