...
class BBoxUtility(object):
    ...
    # 一、RPN阶段NMS的局限性
    # 类别无关性：RPN阶段的NMS主要是基于候选框之间的重叠程度（IoU）来去除冗余的。
    # 它并不考虑候选框的具体类别信息，只是简单地根据IoU阈值来判断哪些候选框是冗余
    # 的。因此，即使两个候选框的重叠度很高，但如果它们属于不同的类别，RPN阶段的NMS
    # 也不会将它们都去除。
    # 初步筛选：RPN阶段的NMS主要是作为一个初步筛选的过程，它的目的是从大量的候选框
    # 中挑选出一定数量的高质量候选框，供后续的分类和回归任务使用。然而，由于它只考
    # 虑了重叠程度和一定的数量限制（如每张图大约保留2000个候选框），所以仍可能保留
    # 一些对于最终检测结果来说是冗余的候选框。
    #
    # 二、分类阶段NMS的必要性
    # 类别特异性：与RPN阶段不同，分类阶段的NMS是基于候选框的类别信息进行的。它会对
    # 每个类别的候选框分别进行NMS处理，以去除那些在同一类别下重叠度较高且置信度较低
    # 的候选框。这样可以确保最终检测结果的每个类别都只保留最佳的候选框。
    # 提高检测精度：分类阶段的NMS能够进一步减少冗余的候选框，提高检测的精度。由于它
    # 考虑了候选框的类别信息和置信度得分，所以能够更准确地判断哪些候选框是真正有意义
    # 的，哪些只是由于重叠而产生的冗余。
    # 适应复杂场景：在实际的目标检测任务中，图像中的目标往往呈现出复杂多样的形态和分
    # 布。因此，仅仅依靠RPN阶段的NMS可能无法完全去除所有的冗余候选框。而分类阶段的NMS
    # 则能够针对每个类别的具体情况进行更精细的处理，从而适应更复杂的检测场景。
    def detection_out(self, predictions, _mbox_priorbox, num_classes, keep_top_k=300,
        ...
        # 先验框
        mbox_priorbox = _mbox_priorbox
        results = []
        # 对每一个图片进行处理
        for i in range(len(mbox_loc)):
            results.append([])
            decode_bbox = self.decode_boxes(mbox_loc[i], mbox_priorbox)
            # 这里是检测每一个框中是否有物体以及置信度
            for c in range(num_classes):
                c_confs = mbox_conf[i, :, c]
                # 如果物体置信度大于门限值，那么就认为这个框中是有物体的
                c_confs_m = c_confs > confidence_threshold
                if len(c_confs[c_confs_m]) > 0:
                    # 取出得分高于confidence_threshold的框
                    # 也就是取出框中有物体并且置信度还比较好的框
                    ...
                    # 取出在非极大抑制中效果较好的内容
                    # 即非极大值抑制的主要目的是去除冗余的边界框，即那些与最高置
                    # 信度边界框重叠度较高（即IoU值超过一定阈值）的边界框，只保
                    # 留最佳的一个边界框，从而提高检测结果的准确性和效率。
