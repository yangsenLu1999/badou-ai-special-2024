...
if __name__ == "__main__":
    ...
    for i in range(EPOCH):
        ...
        while True:
            ...
            # 这句代码包含了
            # 当你调用 model_rpn.train_on_batch(X, Y) 时，你实际上是在执行以下步骤：
            # 前向传播：模型接收输入数据 X，通过模型的各个层进行前向传播，最终计算出输出。
            # 损失计算：使用在 compile 方法中指定的损失函数，计算模型输出 Y_pred 与实际目标 Y 之间的损失。
            # 误差反向传播：根据损失函数的梯度（即损失关于模型参数的偏导数），通过模型的各个层进行反向传播。这个过程会计算出每个参数对损失的影响程度。
            # 权重更新：使用在 compile 方法中指定的优化器，根据反向传播得到的梯度来更新模型的权重。这通常是通过从当前权重中减去梯度的一个比例（这个比例由学习率决定）来实现的。
            # 返回损失值：train_on_batch 方法返回该批次数据上的损失值（或损失值和指标值的列表/元组，如果你在 compile 方法中指定了 metrics）。
