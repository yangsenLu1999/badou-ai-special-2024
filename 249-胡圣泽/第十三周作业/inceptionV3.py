...
"""#
   InceptionV3的网络部分
   改进部分：在InceptionV2：用两个3*3代替5*5和用1*n+n*1的基础上加入了7*7尺寸的卷积核大小（同样用1*7+7*1的两层代替）
   掌握思想：通过分解成小卷积，可以有效降低参数量，防止过拟合
"""
...
"""
将Conv和BatchNormalization和激活定义为一个层，方便网络结构更清晰
"""
def conv2d_bn(x,
              filters,
              num_row,
