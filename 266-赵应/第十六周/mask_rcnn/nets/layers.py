...
def apply_box_deltas_graph(boxes, deltas):
    """
    将先验框转换为预测框。
    先验框/预选框：在原图每个anchor上画的n个方框
    预测框：经过rpn网络预测后的方框，最后需要画到原图上
    :param boxes: 预选框, 格式[y1, x1, y2, x2]
    :param deltas: rpn网络预测的预选框坐标，相对于特征图，范围为0~1。格式为：左上角坐标以及高宽
    :return:
    """
    # 计算先验框中心以及宽高
    width = boxes[:, 3] - boxes[:, 1]
    height = boxes[:, 2] - boxes[:, 0]
    center_y = boxes[:, 0] + .5 * height
    center_x = boxes[:, 1] + .5 * width
    # 计算预测框中心以及宽高
    center_y += deltas[:, 0] + .5 * height
    center_x += deltas[:, 1] + .5 * width
    height *= tf.exp(deltas[:, 2])
    width *= tf.exp(deltas[:, 3])

    # 计算预测框左上角坐标以右下角坐标
    y1 = center_y - .5 * height
    x1 = center_x - .5 * width
    y2 = y1 + height
    x2 = x1 + width
...
class ProposalLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        rpn_class, rpn_bbox, anchors = inputs
        ...
        # 获取前6000个预测框的得分，便于进行非极大值抑制
        # tf.gather(X, Y)根据y中提供的索引将x中的数据取出拼接为新的张量
        ...
        # 获取先验框
        pre_nms_anchors = utils.batch_slice([anchors, ix], lambda x, y: tf.gather(x, y), self.config.IMAGES_PER_GPU,
        ...
        result = utils.batch_slice([boxes, scores], nms, self.config.IMAGES_PER_GPU)
        return result
