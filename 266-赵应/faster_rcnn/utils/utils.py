...
class BBoxUtility(object):
    def __init__(self, priors=None, overlap_threshold=.7, ignore_threshold=.3, nms_threshold=.7, top_k=300):
        self.priors = priors
        ...
        self.nms = tf.image.non_max_suppression(self.boxes, self.scores, self._top_k, iou_threshold=self._nms_threshold)
    ...
    def nms_threshold(self, value):
        assert value, 'value can`t be None'
    ...
    def top_k(self, value):
        assert value, 'value can`t be None'
        self._top_k = value
        self.nms = tf.image.non_max_suppression(self.boxes, self.scores, self._top_k, self._nms_threshold)

    def cal_iou(self, box):
        # 计算出每个真实框与所有的先验框的iou
        # 判断真实框与先验框的重合情况
        inter_up_left = np.maximum(self.priors[:, :2], box[:2])
        inter_bot_right = np.minimum(self.priors[:, 2:4], box[2:])
    ...
    def encode_box(self, box, return_iou=True):
        ...
        # 再计算重合度较高的先验框的中心与长宽
        assigned_priors_center = .5 * (assigned_priors[:, :2] + assigned_priors[:, 2:4])
    ...
    def ignore_box(self, box):
        iou = self.cal_iou(box)
        ignore_box = np.zeros((self.num_priors, 1))
        ...
        if not  assign_mask.any():
            assign_mask[iou.argmax()] = True
        ignore_box[:, 0][assign_mask] =iou[assign_mask]
        return ignore_box.ravel()

    def assign_box(self, boxes, anchors):
        ...
        # 对每一个真实框都进行iou计算
        ignored_boxes = np.apply_along_axis(self.ignore_box, boxes[:, :4])
        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    idx = self.session.run(self.nms, feed_dict=feed_dict)
                    # 取出在非极大抑制中效果较好的内容
    ...
    def nms_for_out(self, all_labels, all_confs, all_bboxes, num_classes, nms):
        ...
        for c in range(num_classes):
            ...
            if len(all_confs[mask]) > 0:
                ...
                feed_dict = {self.boxes: boxes_to_process,
                             self.scores: confs_to_process}
                idx = self.session.run(nms_out, feed_dict=feed_dict)
                # 取出在非极大抑制中效果较好的内容
                good_boxes = boxes_to_process[idx]
                confs = confs_to_process[idx][:, None]
                # 将label、置信度、框的位置进行堆叠。
