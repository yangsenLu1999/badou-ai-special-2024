...
def load_weights(var_list, weight_file):
    ...
    while i < len(var_list) - 1:
        var1 = var_list[i]
        var2 = var_list[i+1]
        if 'conv2d' in var1.name.split('/')[-1]:
            if 'batch_normalization' in var2.name.split('/')[-2]:
                ...
                for var in batch_norm_vars:
                    ...
                    prt += num_params
                    assign_ops.append(tf.assign(var,var_weights,validata_shape=True))
                ...
                ptr += bias_params
                assign_ops.append(tf.assign(bias, bias_weights,validata_shape=True))
            ...
            ptr += num_params
            assign_ops.append(tf.assign(var1,var_weights,validata_shape=True))
            i += 1
