...
from my_yolo_predictor import yolo_predictor
from PIL import Image, ImageFont, ImageDraw
from utils import letterbox_image, load_weights

def mydetect(image_path, model_path, yolo_weights = None):
    """
        Introduction
        ------------
            加载模型，进行预测
...
if '__main__' == __name__:
    if config.pre_train_yolo3:
        ...
        mydetect(config.image_file, config.model_dir, config.yolo3_weights_path)
    # 当使用自训练模型的时候
    else:
        mydetect(config.image_file, config.model_dir)
