...
def conv2d_bn(x,
    ...
    :param x:
    :param filters: 卷积核数量
    :param num_row: 每个卷积核的高
    :param num_col: 每个卷积核的宽
    :param strides: 卷积步长
    :param padding: 卷积填充方式
    :param name: 卷积层名称
    :return:
    """
    if name is not None:
...
def InceptionV3(input_shape=[299, 299, 3],
    ...
    # 通过两次循环，构建两个相同的混合层，每个混合层包含四个分支。
    # 每个分支使用不同的卷积结构。最后，将四个分支的输出进行合并。
    for i in range(2):
        branch1x1 = conv2d_bn(x, 192, 1, 1)
    ...
    # 将输入数据x通过全连接层处理后，得到classes个类别的预测结果
    x = Dense(classes, activation='softmax', name='predictions')(x)

    inputs = img_input

    # 定义一个名为inception_v3的模型
...
if __name__ == '__main__':
    model = InceptionV3()
    # 保存模型
    # model.save_weights("inception_v3_weights_tf_dim_ordering_tf_kernels1.h5")
    # 加载模型
