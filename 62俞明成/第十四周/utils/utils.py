...
class BBoxUtility(object):
    def __init__(self, priors=None, overlap_threshold=0.7, ignore_threshold=0.3,
        ...
        # 先验框,[[x,y,w,h],[x,y,w,h],[x,y,w,h],[x,y,w,h]]
        # number,xywh
        self.priors = priors
        self.num_priors = 0 if priors is None else len(priors)  # 先验框的数量
        self.overlap_threshold = overlap_threshold  # 重叠阈值，用于判断两个框是否重叠
        self.ignore_threshold = ignore_threshold  # 忽略阈值，用于判断一个先验框是否应该被忽略掉
        self._nms_thresh = nms_thresh  # 非极大值抑制阈值
        self._top_k = top_k  # 输出框数量
        self.boxes = tf.placeholder(dtype='float32', shape=(None, 4))  # 输入框
        self.scores = tf.placeholder(dtype='float32', shape=(None,))  # 置信度
        # 极大值抑制
    ...
    def encode_box(self, box, return_iou=True):
        ...
        # 计算真实框宽高的对数比，缩放宽高的对数比
        encoded_box[:, 2:4][assign_mask] = np.log(box_wh / assigned_priors_wh)
        encoded_box[:, 2:4][assign_mask] *= 4
        return encoded_box.ravel()  # 返回编码结果
    ...
    def ignore_box(self, box):
        ...
        # 只给予高于忽略阈值和重合程度小于阈值的先验框一个iou值,其他为0
    ...
    def assign_boxes(self, boxes, anchors):
        ...
        # 标记哪些先验框应该被忽略
        ingored_boxes = np.apply_along_axis(self.ignore_box, 1, boxes[:, :4])  # (num_boxes, num_priors)
        ingored_boxes = ingored_boxes.reshape(-1, self.num_priors, 1)  # (num_boxes, num_priors, 1)
        ignore_iou = ingored_boxes[:, :, 0].max(axis=0)  # (num_priors,)
        # ignore_iou_mask 是一个(num_priors,)的布尔数组，标记哪些先验框应该被忽略
        ignore_iou_mask = ignore_iou > 0
        # 将忽略的先验框的iou设置为-1
        assignment[:, 4][ignore_iou_mask] = -1

        encoded_boxes = np.apply_along_axis(self.encode_box, 1, boxes[:, :4])  # (n, num_priors, 5)
        # 每一个真实框的编码后的值，和iou
        encoded_boxes = encoded_boxes.reshape(-1, self.num_priors, 5)  # (n, num_priors)
