...
from keras._tf_keras.keras.layers import Conv2D, BatchNormalization, Activation, MaxPooling2D, AveragePooling2D, \
    GlobalAveragePooling2D, Dense
from keras._tf_keras.keras.applications.imagenet_utils import decode_predictions


def conv2d_bn(input, output, kernel, stride=(1, 1), padding='same', name=None):
    if name is not None:
        bn_name = name + '_bn'
        ...
        bn_name = None
        conv_name = None
    x = Conv2D(output, kernel, strides=stride, padding=padding, use_bias=False, name=conv_name)(input)
...
# hwc
def InceptionV3(input_size=[299, 299, 3], classes=1000):
    img_input = Input(input_size)
    x = conv2d_bn(img_input, 32, (3, 3), stride=(2, 2), padding='valid')
    x = conv2d_bn(x, 32, (3, 3), stride=(1, 1), padding='valid')
    x = conv2d_bn(x, 64, (3, 3), stride=(1, 1))
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    x = conv2d_bn(x, 80, (1, 1), stride=(1, 1), padding='valid')
    x = conv2d_bn(x, 192, (3, 3), stride=(2, 2), padding='valid')
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
    ...
    branch3x3 = conv2d_bn(x, 384, (3, 3), stride=(2, 2), padding='valid')

    branch3x3dbl = conv2d_bn(x, 64, (1, 1))
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, (3, 3))
    branch3x3dbl = conv2d_bn(branch3x3dbl, 96, (3, 3), stride=(2, 2), padding='valid')
    ...
    branch3x3 = conv2d_bn(branch3x3, 320, (3, 3), stride=(2, 2), padding='valid')

    branch7x7x3 = conv2d_bn(x, 192, (1, 1))
    branch7x7x3 = conv2d_bn(branch7x7x3, 192, (1, 7))
    branch7x7x3 = conv2d_bn(branch7x7x3, 192, (7, 1))
    branch7x7x3 = conv2d_bn(branch7x7x3, 192, (3, 3), stride=(2, 2), padding='valid')
