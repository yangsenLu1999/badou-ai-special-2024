...
class yolo_predictor:
    ...
    def _get_anchors(self):
        ...
        with open(anchors_path, 'r', encoding='latin-1') as f:
            anchors = f.readline()
    ...
    def boxes_and_scores(self, feats, anchors, classes_num, input_shape, image_shape):
        ...
        # [batch_size,h,w,num_anchors, num_classes]
        # batch_size*h*w*num_anchors个框，每个框对应每个类别的概率
    ...
    def _get_feats(self, feats, anchors, num_classes, input_shape):
        ...
        # [1, 1, 1, num_anchors, 宽高]
        # 广播机制，anchors_tensor扩展到与predictions相同的形状
        anchors_tensor = tf.reshape(tf.constant(anchors, dtype=tf.float32), [1, 1, 1, num_anchors, 2])
        grid_size = tf.shape(feats)[1:3]
        # 5 包含偏移量（x, y）、宽高（w, h）和置信度（confidence）
        predictions = tf.reshape(feats, [-1, grid_size[0], grid_size[1], num_anchors, num_classes + 5])

        # 这里构建13*13*1*2的矩阵，对应每个格子加上对应的坐标
        # grid_size=13,tf.reshape(...,[-1, 1, 1, 1]) --> [13,1,1,1]
        # tf.tile(...,[1, grid_size[1], 1, 1]) --> [13,13,1,1]
        ...
        # 将x,y坐标归一化，相对网格的位置
        # tf.cast(grid_size[::-1], tf.float32)将grid_size 转换为浮点型，并反转顺序，使其变为 [width, height]。
        box_xy = (tf.sigmoid(predictions[..., :2]) + grid) / tf.cast(grid_size[::-1], tf.float32)
        # 将w,h也归一化
        # 将实际尺度与先验框的尺寸相乘，得到预测框的宽和高。
        box_wh = tf.exp(predictions[..., 2:4]) * anchors_tensor / tf.cast(input_shape[::-1], tf.float32)
        box_confidence = tf.sigmoid(predictions[..., 4:5])  # 取出置信度
        box_class_probs = tf.sigmoid(predictions[..., 5:])  # 取出类别概率
    ...
    def eval(self, yolo_outputs, image_shape, max_boxes=20):
        ...
        # image_shape是实际图片的大小
        # 第一个特征层输出的形状
        # 切片操作选择形状的第二和第三个维度，即 height 和 width。
        # YOLO使用5层卷积，每层卷积将输入的大小减少一半。因此，总的下采样系数是 32（2^5 = 32）
        # 乘以 32 得到了原始输入图像的大小
