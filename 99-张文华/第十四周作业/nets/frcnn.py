'''
用于存放Faster-R-CNN网络模型
'''

from resnet import ResNet50, classifier_layers
from tensorflow.keras.layers import Conv2D,Input,TimeDistributed,Flatten,Dense,Reshape
...
def get_rpn(base_layers, num_anchors):
    ...
    x_regr = Conv2D(num_anchors*4, (1, 1), activation='linear', kernel_initializer='zero', name='rpn_out_reress')(x)
...
def get_classifier(base_layers, input_rois, num_rois, nb_classes=21, trainable=False):
    ...
    out_regr = TimeDistributed(
        Dense((nb_classes-1)*4, activation='linear', kernel_initializer='zero'),
        name='dense_regress{}'.format(nb_classes)
    )(out)
...
def fet_predict_model(config, num_classes):
    inputs = Input(shape=(None, None, 3))
    roi_input = Input(shape=(None, 4))
