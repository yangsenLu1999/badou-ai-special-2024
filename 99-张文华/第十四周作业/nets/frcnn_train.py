...
from chapter1.第十四周作业.utils.anchors import get_anchors
...
def cls_loss(ratio=3):
    def _cls_loss(y_true, y_pred):
        # y_true [batch_size, num_anchor, num_classes+t1]
        # y_pred [batch_size, num_anchor, num_classes]
        labels = y_true
        anchor_state = y_true[:, :, -1]  # -t1 是需要忽略的, 0 是背景, t1 是存在目标
        ...
        cls_loss_for_object = K.binary_crossentropy(labels_for_object, classification_for_object)
        ...
        # 计算每个先验框的权重
        cls_loss_for_back = K.binary_crossentropy(labels_for_back, classification_for_back)

        # 标准化，实际上是正样本的数量
        normalizer_pos = tf.where(K.equal(anchor_state, 1))
        normalizer_pos = K.cast(K.shape(normalizer_pos)[0], K.floatx())
        normalizer_pos = K.maximum(K.cast_to_floatx(1.0), normalizer_pos)

        normalizer_neg = tf.where(K.equal(anchor_state, 0))
        normalizer_neg = K.cast(K.shape(normalizer_neg)[0], K.floatx())
        ...
        # 总的loss
        return cls_loss_for_object + cls_loss_for_back
    return cls_loss
...
def smooth_l1(sigma=1.0):
    ...
    def _smooth_l1(y_true, y_pred):
        # y_true [batch_size, num_anchor, 4+t1]
        ...
        # 计算 smooth L1 loss
        # f(x) = 0.5 * (sigma * x)^2          if |x| < t1 / sigma / sigma
        ...
        normalizer = K.maximum(1.0, K.shape(indices)[0])
...
def get_img_output_length(width, height):
    ...
    def get_output_length(input_length):
        ...
        for i in range(4):
            ...
            output_length = (input_length + 2 * padding[i] - filter_sizes[i]) // stride + 1
...
class Generator(object):
    ...
    def get_random_date(self, annotation_line, random=True, jitter=.1, hue=.1, sat=1.1, val=1.1, proc_img=True):
        '''r实时数据增强的随机预处理'''
        ...
        if new_ar < 1:
            ...
            nw = int(scale * w)
            nh = int(nw / new_ar)
        image = image.resize((nh, nh), Image.BICUBIC)
        ...
        x[x > 1] = 1
        x[x < 0] = 0
        image_data = hsv_to_rgb(x) * 255    # numpy array, 0 to t1
    ...
    def generate(self):
        while True:
            ...
            for annotation_line in lines:
                img, y = self.get_random_date(annotation_line)
