...
class FasterRCNNBase(nn.Module):
    ...
    Arguments:
        backbone (nn.Module): 特征提取部分
        rpn (nn.Module): RPN部分
        roi_heads (nn.Module): takes the features + the proposals from the RPN and computes
            detections / masks from it. ROI部分
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,  # backbone特征提取网络
                 # transform parameter
                 min_size=800, max_size=1333,  # 预处理resize时限制的最小尺寸与最大尺寸
                 image_mean=None, image_std=None,  # 预处理normalize时使用的均值和方差
                 # RPN parameters
                 rpn_anchor_generator=None, rpn_head=None,  # 用于生成anchor的生成器
