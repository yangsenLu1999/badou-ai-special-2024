...
def cls_loss(ratio=3):
    def _cls_loss(y_true, y_pred):
        labels         = y_true #[batch_size, num_anchor, num_classes]
        # -1 is need to ignore, 0 is background, 1 is exist object
        anchor_state   = y_true[:,:,-1]

        # get the predict result
        classification = y_pred

        # find the prior box which is object
        ...
        # find the prior box which is background
        ...
        # calculate the number of the positive sample for normalizer
        ...
        # normalize the loss of the positive and negative
...
def smooth_l1(sigma=1.0):
    ...
    def _smooth_l1(y_true, y_pred):
        ...
        # find the positive sample prior box
...
def class_loss_regr(num_classes):
    ...
    def class_loss_regr_fixed_num(y_true, y_pred):
        ...
        loss = 4*keras.backend.sum(y_true[:, :, :4*num_classes] * (x_bool * (0.5 * x * x) + (1 - x_bool) * (x_abs - 0.5))) / keras.backend.sum(epsilon + y_true[:, :, :4*num_classes])
        return loss
...
# using the data augmentation to get the data
class Generator(object):
    ...
    def get_random_data(self, annotation_line, random=True, jitter=.1, hue=.1, sat=1.1, val=1.1, proc_img=True):
        ...
        # correct boxes
        # adjust the box according to the image size
    ...
    def generate(self):
        while True:
            ...
            for annotation_line in lines:  
                ...
                # convert the bbox according to the image size ratio
                ...
                # assign the anchor box according to the bbox
                ...
                # process the positive and negative sample
