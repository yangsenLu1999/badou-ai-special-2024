...
def calc_iou(R, config, all_boxes, width, height, num_classes):
    # ajust the coordinates of the bounding boxes according to the rpn stride
    bboxes = all_boxes[:,:4]
    ...
    # iterate through all the rpn proposals
    for ix in range(R.shape[0]):
        ...
        best_iou = 0.0
        best_bbox = -1
        # calculate the IoU of the proposal with all the ground truth bounding boxes
        ...
        if best_iou < config.classifier_min_overlap:
            ...
            # calculate the width and height of the proposal
            w = x2 - x1
            h = y2 - y1
            x_roi.append([x1, y1, w, h])
            IoUs.append(best_iou)

            # assign the label to the proposal
