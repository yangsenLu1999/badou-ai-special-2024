...
def InceptionV3(input_shape=[299,299,3], classes=1000):
    ...
    branch33 = conv2d_bn(x, 192, 1, 1)
    branch33 = conv2d_bn(branch33, 320, 3, 3, strides=(2, 2), padding='valid')

    branch773 = conv2d_bn(x, 192, 1, 1)
    branch773 = conv2d_bn(branch773, 192, 1, 7)
    branch773 = conv2d_bn(branch773, 192, 7, 1)
    branch773 = conv2d_bn(branch773, 192, 3, 3, strides=(2, 2), padding='valid')

    branch_pool = MaxPooling2D((3, 3), strides=(2, 2))(x)
    x = layers.concatenate([branch33, branch773, branch_pool], axis=3, name='mixed8')

    # Block3 part2 part3
    # 8 x 8 x 1280 -> 8 x 8 x 2048 -> 8 x 8 x 2048
    for i in range(2):
        branch11 = conv2d_bn(x, 320, 1, 1)

        branch33 = conv2d_bn(x, 384, 1, 1)
        branch331 = conv2d_bn(branch33, 384, 1, 3)
        branch332 = conv2d_bn(branch33, 384, 3, 1)
        branch33 = layers.concatenate([branch331, branch332], axis=3, name='mixed9_' + str(i))

        branch33dbl = conv2d_bn(x, 448, 1, 1)
        branch33dbl = conv2d_bn(branch33dbl, 384, 3, 3)
        branch33dbl1 = conv2d_bn(branch33dbl, 384, 1, 3)
        branch33dbl2 = conv2d_bn(branch33dbl, 384, 3, 1)
        branch33dbl = layers.concatenate([branch33dbl1, branch33dbl2], axis=3)

        branch_pool = AveragePooling2D((3, 3), strides=(1, 1), padding='same')(x)
        branch_pool = conv2d_bn(branch_pool, 192, 1, 1)
        x = layers.concatenate([branch11, branch33, branch33dbl, branch_pool], axis=3, name='mixed' + str(9 + i))
    # 平均池化后全连接。
...
if __name__ == '__main__':
    model = InceptionV3()
    model.load_weights("H:/CV/PRE/pythonProject1/data/inceptionv3/inception_v3_weights_tf_dim_ordering_tf_kernels.h5")

    img_path = "H:/CV/PRE/pythonProject1/data/inceptionv3/elephant.jpg"
