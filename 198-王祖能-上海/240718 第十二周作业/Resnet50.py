...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    x = Conv2D(filter1, (1, 1), strides=strides, name=conv_name_base + '2a')(input_tensor)  # filter1表示卷积后维度
    x = BatchNormalization(name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
    # 单个整数或由两个整数构成的list / tuple，卷积核的宽度和长度。如为单个整数，则表示各空间维度的相同长度
    x = Conv2D(filter2, kernel_size, padding='same', name=conv_name_base + '2b')(x)
    x = BatchNormalization(name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)
    x = Conv2D(filter3, (1, 1), name=conv_name_base + '2c')(x)  # padding='valid',
...
if __name__ == '__main__':
    model = resnet50()
    model.summary()  # 显示构建模型的层级关系，bias应该就是不训练的参数
    img1_path = 'elephant.jpg'
    img2_path = 'bike.jpg'
    img = image.load_img(img1_path, target_size=(224, 224))  # 从指定路径加载图像，并将其调整为指定的大小(224, 224)
    print(img.size)
