from dataset_homework import ISBI_Loader
import torch
from torch import nn
from model.Unet_model_homework import Unet

def train_net(net, device, data_path, epochs=40, batch_size=1, lr=0.00001):
    # 加载训练集
    isbi_dataset = ISBI_Loader(data_path)
    train_loader = torch.utils.data.DataLoader(dataset=isbi_dataset, batch_size=batch_size, shuffle=True)
    optimizer = torch.optim.RMSprop(net.parameters(), lr=lr, weight_decay=1e-8, momentum=0.9)
    criterion = nn.BCEWithLogitsLoss()
    best_loss = float('inf')
    for epoch in range(epochs):
        net.train()
        print(f'epoch: {epoch + 1}/{epochs}')
        index = 1
        for images, labels in train_loader:
            optimizer.zero_grad()
            images = images.to(device=device, dtype=torch.float32)
            labels = labels.to(device=device, dtype=torch.float32)
            pred = net(images)
            loss = criterion(pred, labels)
            print(f'image: {index}/30')
            print('Loss/Train: ', loss.item())
            if loss < best_loss:
                best_loss = loss
                torch.save(net.state_dict(), 'best_model_forZhuYiGuang.pth')
                print('Saving best model...')
            loss.backward()
            optimizer.step()
            with open('log.txt', 'a') as f:
                f.write(f'epoch:{epoch + 1}/{epochs}, image: {index}/30' + '\n' + str(loss.item()) + '\n' + '\n')
            index += 1

if __name__ == '__main__':
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = Unet(n_channels=1, n_classes=1)
    net.to(device=device)
    data_path = "data/train/"
    train_net(net, device, data_path)
    with open('log.txt', 'a') as f:
        f.write('Train finished')
