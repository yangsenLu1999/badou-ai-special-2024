from nets.resnet_homework import ResNet50, classifier_layers
from keras.layers import Conv2D, Reshape, TimeDistributed, Flatten, Dense, Input
from nets.RoiPoolingConv_homework import RoiPoolingConv
from resnet_homework import ResNet50
...
def get_model(config, num_classes):
    ...
    model_all = Model([inputs, roi_input], rpn[0:2] + classifier)
    return model_rpn, model_classifier, model_all

def get_perdict_model(config, num_classes):
    inputs = Input(shape=(None, None, 3))
    roi_input = Input(shape=(None, 4))
