...
def _conv_block(inputs, filters, kernel=(3,3), strides=(1,1)):
    x = Conv2D(filters=filters, kernel_size=kernel, strides=strides, padding='same', use_bias=False,name='covnv1')(inputs)
    x = BatchNormalization(name='conv1_bn')(x)
    x = Activation(relu6, name='conv1_relu')(x)
    return x

def _depthwise_conv_block(inputs, pointwise_conv_filters, depth_multiplier=1, strides=(1,1),block_id=1):
    x = DepthwiseConv2D(kernel_size=(3,3), strides=strides, padding='same', depth_multiplier=depth_multiplier, use_bias=False, name='conv_dw_%d' % block_id)(inputs)
...
def MobileNet(input_shape=[224, 224, 3], depth_multiplier=1, dropout=1e-3,classes=1000):
     ...
     x = Conv2D(classes, (1,1), padding='same',name='conv_pred')(x)
