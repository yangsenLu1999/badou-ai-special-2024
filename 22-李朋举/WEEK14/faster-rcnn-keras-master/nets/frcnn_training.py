...
# width, height 原始图像的宽度和高度、img_min_side调整后图像的最小边长
def get_new_img_size(width, height, img_min_side=600):
    if width <= height:  # 如果原始图像的宽度小于或等于高度，则执行以下操作
        f = float(img_min_side) / width  # 计算调整比例，将最小边长除以原始宽度   0.45112781954887216
        resized_height = int(f * height)  # 根据调整比例计算新的高度，并将结果转换为整数  45112781954887216 * 1330 = 600
        resized_width = int(img_min_side)  # 将最小边长设置为新的宽度  600
    else:
