...
def detect(image_path, model_path, yolo_weights=None):
    ...
    resize_image = letterbox_image(image, (416, 416))  # <PIL.Image.Image image mode=RGB size=416x416 at 0x15675768198>
    image_data = np.array(resize_image, dtype=np.float32)  # （416,416,3）
    # 归一化
    image_data /= 255.
    ...
    #   图片输入
    # ---------------------------------------#
    # 创建 input_image_shape 占位符，用于接收原始图像的尺寸
    input_image_shape = tf.placeholder(dtype=tf.int32, shape=(2,))
    # 创建 input_image 占位符，用于接收处理后的图像数据
