...
class yolo:
    """
    初始化
    输入：   norm_decay-预测计算moving average时的衰减率
    ...
    """
    获取类别名字，coco数据集一共80类
    输入：   classes_path-数据集类别对应文件
    输出：   class_names-coco数据集类别对应的名字
    """
    def _get_class(self):
        classes_path = os.path.expanduser(self.classes_path)        # os.path.expanduser() 把path中包含的"~"和"~user"转换成用户目录
        ...
        return class_names                                          # 返回class_names

    """
    获取anchors
    输入：  anchors_path-yolo anchor 文件路径
    输出：  np.array(anchors).reshape(-1, 2)-yolo anchor
    """
    def _get_anchors(self):
        anchors_path = os.path.expanduser(self.anchors_path)        # os.path.expanduser() 把path中包含的"~"和"~user"转换成用户目录
        with open(anchors_path) as f:                               # 打开文件
            anchors = f.readline()                                  # 读取一行
        anchors = [float(x) for x in anchors.split(',')]            # split()通过指定分隔符对字符串进行切片
        return np.array(anchors).reshape(-1, 2)                     # reshape(-1, 2) 代表行数不知道，列数为2
    ...
    """
    对权重进行l2正则化，防止过拟合
    输入：  input_layer-输入的四维tensor,name-该层的名字,training-是否为训练过程,norm_decay-方差加上极小的数，防止除以0的情况,norm_epsilon-预测计算moving average时的衰减率
    输出：  tf.nn.leaky_relu(bn_layer, alpha=0.1)-l2正则化之后的feature map,其中alpha为leaky_relu的系数，使用leaky_relu激活函数,防止梯度消失
    """
    ...
    比如，输入图片的大小为416*416，卷积核大小为3，若stride为2时，（416 - 3 + 2）/ 2 + 1， 计算结果为208，相当于做了池化层处理.+2是因为padding的时候，padding的是一维，两边都padding了一维，因此需要加2
    因此需要对stride大于1的时候，先进行一个padding操作, 采用四周都padding一维代替'same'方式
    输入：  inputs-输入的四维tensor,filters_num-卷积核数量,kernel_size-卷积核大小,name-该层的名字,use_bias-是否使用偏置项,strides-卷积步长
    输出：  conv_layer-卷积之后的feature map
    """
    ...
    输入：  inputs-输入的四维tensor,filters_num-卷积核数量,trainging-是否为训练过程,blocks_num-block的数量,conv_index-为了方便加载预训练权重，统一命名序号,weights_dict-加载预训练模型的权重,norm_decay-在预测时计算moving average时的衰减率,norm_epsilon-预测计算moving average时的衰减率
    输出：  conv - 残差卷积之后的feature map  
    """
