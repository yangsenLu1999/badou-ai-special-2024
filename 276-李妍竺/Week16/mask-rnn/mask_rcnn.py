...
class MASK_RCNN(object):
    ...
    def detect_image(self, image):
        ...
        detections, _, _, mrcnn_mask, _, _, _ = self.model.predict([molded_images, image_metas, anchors],
                                                                   verbose=0)

        final_rois, final_class_ids, final_scores, final_masks = unmold_detections(
            detections[0], mrcnn_mask[0], image[0].shape, molded_images[0].shape,
            windows[0]
        )
        ...
        visualize.display_instances(image[0], r["rois"], r["masks"], r["class_ids"], self.class_names,
                                    r["scores"])
