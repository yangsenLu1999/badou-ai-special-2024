...
if __name__ == '__main__':
    ...
    # 每次训练一个世代都会保存
    callbacks = [
        keras.callbacks.TensorBoard(log_dir=MODEL_DIR, histogram_freq=0, write_graph=True,
                                    write_images=False),
        keras.callbacks.ModelCheckpoint(os.path.join(MODEL_DIR, "epoch{epoch:03d}_loss{loss:.3f}_val_loss"
                                                                "{val_loss:.3f}.h5"),
                                        verbose=0, save_weights_only=True)
    ]

    log(f"\nStarting at epoch {init_epoch}. LR={learning_rate}\n")
    log(f"Checkpoint Path: {MODEL_DIR}")
    ...
    model.fit_generator(train_generator, initial_epoch=init_epoch, epochs=epoch,
