...
def clip_boxes_graph(boxes, window):  #剪切框图
    """
    boxes: [N, (y1,x1,y2,x2)]
    window: 以y1,x1,y2,x2的形式
    """
    # split
...
class ProposalLayer(Layer):  # 建议层
    ...
    def call(self, inputs, **kwargs):
        # 代表这个先验框内部是否有物体[batch_size, num_rois, 1]
        scores = inputs[0][:, :, 1]

        # 代表这个先验框的调整参数[batch_size, num_rois, 4]
        deltas = inputs[1]
        ...
        # 获得这些框的得分(这个函数只是为了应付显存不足的情况，
        # 若没这种情况，可直接使用tf.gather或者也可以使用上面那一行最后改成.values)
...
def parse_image_meta_graph(meta):
    """将meta里面的参数进行分割"""
    # utils.compute_image_meta
    image_id = meta[:, 0]
    original_image_shape = meta[:, 1:4]
    image_shape = meta[:, 4:7]
    window = meta[:, 7:11]  # (y1,x1,y2,x2)以像素为单位的图像窗口
    scale = meta[:, 11]
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    :param rois:[N, (y1, x1, y2, x2)]在归一化坐标中
    :param probs:[N, num_classes]类别概率
    :param deltas:[N, num_classes, (dy, dx, log(dh), log(dw))]特定类别，边界框增量
    :param window:(y1, x1, y2, x2)在归一化坐标中。图像的部分包含，不包括填充的图像。
    :return:检测整形：[num_detections，（y1，x1，y2，x2，class_id，score）]其中坐标被归一化。
    """
    # 找到得分最高的类
    ...
    # 进行解码
    # Shape: [boxes, (y1, x1, y2, x2)] 在归一化坐标中
    # 常见做法，得分最高的类的9个边界框的调整值作用于所有边界框的坐标（自动广播）
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        # 索引交集，既不是背景，又大于config.DETECTION_MIN_CONFIDENCE
    ...
    # 确保每个类别只出现一次，以便在后续的操作中可以按独立的类别进行处理，而不会重复处理相同的类别
    ...
    top_ids = tf.nn.top_k(class_scores_keep, k=num_keep, sorted=True)[1]  # 相当于.indices
    keep = tf.gather(keep, top_ids)
    # 将输出排列为[N，（y1，x1，y2，x2，class_id，score）] *****
    detections = tf.concat([
        tf.gather(refined_rois, keep),
        tf.to_float(tf.gather(class_ids, keep))[..., tf.newaxis],  # 这两个参数都是[config.DETECTION_MAX_INSTANCES]
        tf.gather(class_scores, keep)[..., tf.newaxis]  # 而第一个参数是[N, (y1,x1,y2,x2)],所以需要newaxis
    ], axis=1)

    # 如果达不到数量就padding
    gap = config.DETECTION_MAX_INSTANCES - tf.shape(detections)[0]
    detections = tf.pad(detections, [(0, gap), (0, 0)], "CONSTANT")  # 第一个维度（行）填充gap个
    return detections
...
class DetectionLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        detections_batch = utils.batch_slice([rois, mrcnn_class, mrcnn_bbox, window],
                                             ...
                                             self.config.IMAGES_PER_GPU)
        # Reshape output
        # [batch, num_detections, (y1, x1, y2, x2, class_id, class_score)] 归一化坐标中
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # 处理COCO人群
    # COCO中的群组框是围绕多个实例的边界框。排除他们的训练。群组框被赋予负类ID。
    ...
    # 计算建议框和crowd boxes的重合程度[proposals, crowd_boxes]
    ...
    # 2.负样本建议框和真实框的重合程度小于0.5，并且不属于crowds
    ...
    # 解码获得网络应该有的预测结果(所有正样本与真实框的deltas)
    deltas = utils.box_refinement_graph(positive_rois, roi_gt_boxes)
    deltas /= config.BBOX_STD_DEV

    # 切换mask的形式[height, width, N] -> [N, height, width, 1]
...
class DetectionTargetLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        # 对真实框进行编码
        names = ["rois", "target_class_ids", "target_boxes", "target_masks"]
